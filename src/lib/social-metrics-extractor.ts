...
export async function extractSocialMetrics(
  ...
  const fetchChannelData = async (
    ...
    // Extracted connection metadata values
    ...
    const metaComments = Number(meta.comments ?? 0);
    const metaShares = Number(meta.shares ?? 0);
    const metaInteractions = metaLikes + metaComments + metaShares;
    const metaEngRate = (meta.engagementRate as string) || (metaReach > 0 ? `${((metaInteractions / metaReach) * 100).toFixed(1)}%` : '0.0%');
  ...
  activeChannelResults.forEach((c) => {
    ...
    if (c.recentPosts && c.recentPosts.length > 0) {
      ...
      // Use exact real channel indicators extracted from connection / API
      ...
      allPosts.push({
        ...
        title: `Conteúdo / Canal — ${c.accountName}`,
