      ...
      <header className="flex items-center gap-4 p-5 border-b border-white/[0.06] bg-white/[0.02]">
        <div className="relative">
          <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-[#FF6A00]/30 shadow-[0_0_15px_rgba(255,106,0,0.3)]">
             <div className="w-full h-full bg-gradient-to-br from-[#FF6A00] to-[#FF3A00] flex items-center justify-center">
        ...
        <div>
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            Lucca <Sparkles className="w-4 h-4 text-[#FF6A00]" />
          </h2>
          <p className="text-[13px] text-emerald-400 font-medium">Sempre Online</p>
        </div>
      </header>
          ...
          <div key={msg.id} className={`flex items-start gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
            ...
            <div className={`w-10 h-10 shrink-0 rounded-full flex items-center justify-center border ${
              msg.role === 'user' 
                ...
                : 'bg-gradient-to-br from-[#FF6A00]/20 to-[#FF3A00]/10 border-[#FF6A00]/30'
            }`}>
              {msg.role === 'user' ? <User className="w-5 h-5 text-white/70" /> : <Bot className="w-5 h-5 text-[#FF6A00]" />}
            </div>
