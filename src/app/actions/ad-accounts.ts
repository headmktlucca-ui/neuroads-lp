...
export async function listGoogleAdsAccounts(accessToken: string) {
  try {
          ...
          if (customer) {
            ...
            if (customer.manager) {
              ...
              if (clientRes.ok) {
                ...
                (clientData.results as GoogleAdsClientResult[] | undefined || []).forEach((res) => {
                  const client = res.customerClient;
                  if (!client?.clientCustomer) return;
    ...
    return { success: false, error: getErrorMessage(error, 'Erro interno ao processar contas do Google Ads.') };
  }
}
...
export async function saveConnection(userId: string, platform: string, accountId: string, accessToken: string, loginCustomerId?: string) {
  try {
    ...
    return { success: false, error: getErrorMessage(error, 'Erro ao salvar conexão.') };
  }
}
...
export async function removeConnection(userId: string, platform: string) {
  try {
    ...
    return { success: false, error: getErrorMessage(error, 'Erro ao remover conexão.') };
  }
}
