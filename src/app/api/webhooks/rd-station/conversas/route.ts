...
export async function GET() {
  return NextResponse.json({
    ok: true,
    provider: 'rd-station-conversas',
    mode: 'webhook',
  });
}

export async function POST(request: NextRequest) {
  const verifyToken = process.env.RD_CONVERSAS_WEBHOOK_VERIFY_TOKEN?.trim();
  const providedToken =
  ...
  try {
    ...
    console.error('Falha ao salvar webhook RD Station Conversas:', error);
